...
7 тематических блоков (1, 2, 3, 4, 4b, 5, 6, 7), в каждом 3-5 вопросов. Спрашивай **только то, что нельзя узнать из кода**. Если в discovery не нашли подсистему — пропускай блок.

**КРИТИЧНО — без исключений:**

1. **Каждый блок задаётся одним вызовом `AskUserQuestion`** со структурированными опциями. Свободный текст в чате с нумерованным списком вопросов **ЗАПРЕЩЁН** — это нарушает контракт скилла (теряются `source`-метки в `.onboarding-context.json`) и ломает регенерацию.
2. **Перед первым блоком обязательно** вызови `ToolSearch(query="select:AskUserQuestion", max_results=1)` — это deferred tool, его схема не загружена по умолчанию.
3. **Не изобретай новые блоки.** Если кажется что нужен блок "Контекст команды" или "О проекте" — этого нет в скилле, такая информация либо извлекается из discovery, либо не нужна. Используй блоки 1–7 из `phases/02-interview.md` дословно.
4. **Один блок = один tool call.** Не дроби, не объединяй несколько блоков в один.

Если по какой-то причине `AskUserQuestion` недоступен после `ToolSearch` — остановись и сообщи пользователю, не переходи на свободный текст.
