...
7 тематических блоков (1, 2, 3, 4, 4b, 5, 6, 7). **Спрашивай только то, что не извлёк из кода.** Каждый блок задаётся **одним вызовом `AskUserQuestion`** с несколькими структурированными вопросами одновременно.

> ⛔ **СТОП-СИГНАЛ.** Если ты собираешься написать пользователю что-то вроде:
> "Блок 1 — ... \n 1. Вопрос? \n 2. Вопрос? \n Ответь сразу на все" — **ты нарушаешь скилл**. Вернись назад, вызови `ToolSearch(query="select:AskUserQuestion", max_results=1)` и используй `AskUserQuestion` tool. Свободный текст вместо tool call ломает сохранение `source` в `.onboarding-context.json` и делает регенерацию (`/onboarding-suite:update`) невозможной.
>
> **Также запрещено** изобретать собственные блоки ("Контекст команды", "О проекте", "Знакомство"). Блоки фиксированы — 1, 2, 3, 4, 4b, 5, 6, 7 ниже. Если информации в блоках не хватает — она либо в discovery, либо не нужна.

## Шаг 0 (обязательно перед первым блоком): загрузить AskUserQuestion

В текущей версии Claude Code `AskUserQuestion` — **deferred tool**: его схема не доступна по умолчанию, попытка вызвать его без загрузки даст `InputValidationError`. Перед первым блоком интервью **обязательно** вызови:

```
ToolSearch(query="select:AskUserQuestion", max_results=1)
```

После этого `AskUserQuestion` появится в твоём tool registry и его можно использовать как обычный tool. Если `ToolSearch` ничего не вернёт (старая версия харнеса) — `AskUserQuestion` уже загружен по умолчанию, можно идти дальше.

**Запрещено** заменять `AskUserQuestion` на свободный список вопросов в тексте — это нарушает контракт скилла и ломает сохранение `source` в `.onboarding-context.json`.
...
## Блок 4b — Логи и комментарии

**Всегда задаём.** Влияет на `team_decisions.logging` и `team_decisions.comment_style`, которые попадут в CLAUDE.md (§8 Observability, §4 Naming).
...
```json
{
  "questions": [
    ...
    {
      "header": "Log levels",
      "question": "Политика уровней логов: что и где должно логироваться?",
      "multiSelect": false,
      "options": [
        {"label": "INFO бизнес, DEBUG трейс", "description": "INFO — бизнес-события, DEBUG — детали, WARN — recoverable, ERROR — unrecoverable"},
        {"label": "Только WARN/ERROR в prod", "description": "Минимум шума, INFO/DEBUG только локально/в dev"},
        {"label": "Verbose в dev, INFO в prod", "description": "Уровень управляется профилем"},
        {"label": "Другое", "description": "Опишу свободно"}
      ]
    },
    {
      "header": "Log content",
      "question": "Что обязательно логировать на границах сервиса?",
      "multiSelect": true,
      "options": [
        {"label": "Входящие HTTP запросы", "description": "method, path, status, latency на INFO"},
        {"label": "Исходящие HTTP вызовы", "description": "host, status, latency"},
        {"label": "Kafka produce/consume", "description": "topic, partition, offset, key"},
        {"label": "Бизнес-события", "description": "payment.created, refund.completed и т.п."},
        {"label": "Ничего обязательного", "description": "На усмотрение автора"}
      ]
    },
    {
      "header": "Comment style",
      "question": "Стиль комментариев в коде",
      "multiSelect": false,
      "options": [
        {"label": "Минимум — только WHY", "description": "Комментарии только для неочевидной мотивации/инвариантов; код самодокументирующийся"},
        {"label": "Javadoc на public API", "description": "Javadoc обязателен для public методов/классов, inline комментарии редко"},
        {"label": "Javadoc везде", "description": "Javadoc на public + protected, inline для сложной логики"},
        {"label": "Свободный стиль", "description": "Нет формальных правил, на усмотрение автора"}
      ]
    },
    {
      "header": "Comment lang",
      "question": "Язык комментариев и Javadoc",
      "multiSelect": false,
      "options": [
        {"label": "English only", "description": "Все комментарии и Javadoc — на английском"},
        {"label": "Russian", "description": "На русском (внутренний проект)"},
        {"label": "Mixed", "description": "Javadoc EN, inline RU допустимо"}
      ]
    }
  ]
}
...
"team_decisions": {
  ...
  "logging": {
    "value": {
      "requirements": ["Structured (JSON)", "Correlation ID", "Маскирование PII/PCI"],
      "levels_policy": "INFO бизнес, DEBUG трейс",
      "must_log": ["Входящие HTTP запросы", "Бизнес-события", "Kafka produce/consume"]
    },
    "source": "user-confirmed"
  },
  "comment_style": {
    "value": {
      "style": "Минимум — только WHY",
